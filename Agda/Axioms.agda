module Axioms where
  ...
  postulate
    ...
    -- Set extensionality
    extensionality : ∀ (A B : set) → (∀ (x : set) → x ∈ A ↔ x ∈ B) → A ≡ B

    -- Empty set.
    ∅ : ∃[ B ] ∀ (x : set) → ¬ (x ∈ B)

    -- Paring axiom.
    ⟨_,_⟩ : ∀ (u v : set) → ∃[ B ] ∀ (x : set) → x ∈ B ↔ x ≡ u ⊎ x ≡ v

    -- Arbitray union.
    ⋃ : ∀ (A : set) → ∃[ B ] ∀ (x : set) → x ∈ B ↔ ∃[ b ] b ∈ A × x ∈ b

    -- Subset schema.
    comprehension : ∀ (c : set) (ϕ : set → Set) → ∃[ B ] ∀ (x : set) → x ∈ B ↔ x ∈ c × ϕ x
